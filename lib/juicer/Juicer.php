...
class Juicer
{
  ...
  public function juice($infile)
  {
    ...
    if ($this->cli)
    {
      if ($this->cli->getOption('list', false))
      {
  ...
  protected function requireFile($srcFile, $from)
  {
    ...
    if ($handle)
    {
      ...
      while (!feof($handle))
      {
        ...
        {
          $buffer = $this->parseCommand(substr($buffer, 4), $from, $srcFile, $lineNr);
        }
  ...
  protected function parseCommand($buffer, &$from, $srcFile, $lineNr)
  {
    ...
    {
      $this->throwException('Improperly formatted Juicer command at line %d ? ==> %s', $lineNr, $buffer);
    }
  ...
  {
    $count = 0;
    $methodName = preg_quote($methods[0]);
